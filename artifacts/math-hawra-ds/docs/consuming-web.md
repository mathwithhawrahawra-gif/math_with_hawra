# Consuming Math With Hawra Design System in web apps

Read `artifacts/math-hawra-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/math-hawra-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/math-hawra-ds/styles.css";
```
...
`node_modules/@workspace/math-hawra-ds/src/components` to `content`.
...
import { cn } from "@workspace/math-hawra-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/math-hawra-ds/hooks/use-toast";
```
...
`@workspace/math-hawra-ds/components/ui/button`. Run the app's typecheck
