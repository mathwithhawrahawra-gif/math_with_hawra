    ...
    <nav aria-label="Design system navigation" className="space-y-5 py-2">
      {showOverview ? (
        <button
          ...
          className="block w-full rounded-xl px-3 py-2 text-right text-sm font-bold transition-colors hover:bg-muted aria-[current=true]:bg-primary aria-[current=true]:text-primary-foreground"
        >
          {OVERVIEW_ENTRY.name}
        </button>
      ) : null}
      ...
      {groups.map((group) => (
        <div key={group.name}>
          ...
          <div className="mt-2 space-y-1 border-r pr-2">
            {group.entries.map((entry) => (
              <button
                ...
                className="block w-full rounded-xl px-3 py-1.5 text-right text-sm transition-colors hover:bg-muted aria-[current=true]:bg-primary aria-[current=true]:text-primary-foreground"
              >
                {entry.name}
              </button>
            ))}
...
export function DesignSystemBrowser() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-secondary/10 text-foreground md:grid md:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="border-b bg-muted/20 md:sticky md:top-0 md:flex md:h-screen md:flex-col md:border-b-0 md:border-l">
